---
name: code-review-claude-no-autopost
description: Review a GitHub pull request from a git worktree using parallel Claude subagents pinned to specific model tiers (Sonnet for analysis, Haiku for scoring). Returns findings to the user and never auto-posts comments on the PR. Use whenever the user says "review this PR", "code review", "check this pull request", or pastes a PR URL.
---

# Code review (Claude Code — worktree, pinned tiers, no auto-post)
...
1. **Eligibility check** — Use a Haiku agent to check if the PR (a) is closed, (b) is a draft, (c) does not need a review (automated, trivial, or already reviewed by you). If so, stop and tell the user.

2. **Gather CLAUDE.md files** — Use another Haiku agent to list file paths of any relevant CLAUDE.md files: the root CLAUDE.md, plus any in directories the PR modified.

3. **Summarize the PR** — Use a Haiku agent to view the PR and return a summary of the change.

4. **Parallel review (5 Sonnet agents)** — Launch 5 parallel Sonnet agents to independently review the change. Each returns a list of issues and why each was flagged:
...
5. **Score issues** — For each issue from step 4, launch a parallel Haiku agent with the PR, issue description, and CLAUDE.md files (from step 2). Score each 0–100 using this rubric (pass verbatim):

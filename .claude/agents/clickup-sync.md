...
1. Load the token from 1Password, falling back to the macOS keychain: `export CLICKUP_API_TOKEN=$(op read "op://Private/CLICKUP_API_TOKEN/password" 2>/dev/null || security find-generic-password -s CLICKUP_API_TOKEN -a "$USER" -w)`. If `CLICKUP_API_TOKEN` is unset after this, stop and tell the user to add it (`op item create --category=password --title=CLICKUP_API_TOKEN --vault=Private password='<token>'` or `security add-generic-password -U -s CLICKUP_API_TOKEN -a "$USER" -w '<token>'`).
